...
def api_download_report():
    try:
        ...
        cls = data.get('cls', '0')
        rem = data.get('rem', '0')
        pdf_title = data.get('pdfTitle', 'تقرير').strip()
...
<head>
    ...
    <style>
        ...
        body {{ font-family: 'Segoe UI', 'Arial', sans-serif; direction: rtl; text-align: right; padding: 40px; color: #000; background: white; }}
        .header {{ display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; border-bottom: 2px solid #000; padding-bottom: 12px; margin-bottom: 20px; }}
        .right-header {{ text-align: right; font-size: 1.1rem; font-weight: bold; line-height: 1.6; }}
        .middle-header {{ text-align: center; padding: 0 20px; }}
        .middle-header img {{ height: 80px; width: auto; object-fit: contain; }}
        .left-header {{ text-align: left; font-size: 0.95rem; font-weight: bold; line-height: 1.5; font-family: 'Ebrima', sans-serif; }}
        .report-title {{ text-align: center; font-size: 1.5rem; font-weight: bold; margin-top: 20px; margin-bottom: 20px; color: #000; text-decoration: underline; }}
        table {{ width: 80%; margin: 0 auto; border-collapse: collapse; font-size: 1.25rem; }}
        th, td {{ border: 1px solid #000; padding: 10px 15px; text-align: center; }}
        th {{ background-color: #92D050; color: #ffffff; font-weight: bold; }}
        th:last-child {{ background-color: #f3f4f6; color: #000; }}
        td:first-child {{ background-color: #92D050; color: #ffffff; }}
        .footer {{ margin-top: 25px; text-align: left; font-size: 1.1rem; padding-left: 50px; }}
...
<body>
    <div class="header">
        ...
        <div class="middle-header">
            <img src="http://127.0.0.1:5000/static/img/Picture1.png" style="height: 80px; width: auto; object-fit: contain;">
        </div>
    ...
    <div class="report-title">
        نشاط شعبة الخبرة من {start_date} إلى غاية {end_date}
    </div>
    <table>
        <thead>
            <tr>
    ...
    <div class="footer">
        حرر في: {datetime.datetime.now().strftime('%d/%m/%Y')}
    </div>
</body>
</html>
"""
        ...
        safe_title = re.sub(r'[\/:*?"<>|]', '_', pdf_title)
        file_name = f"{safe_title}.html"
        full_path = os.path.join(downloads_path, file_name)
        
        counter = 1
        while os.path.exists(full_path):
            full_path = os.path.join(downloads_path, f"{safe_title}_{counter}.html")
            counter += 1
        ...
        try:
            ...
            write_log(f"[-] Failed to open file: {start_err}")
            
        write_log(f"[+] Saved report: {full_path}")
        return jsonify({"success": True, "path": full_path})
    except Exception as e:
        write_log(f"[-] Error downloading report: {e}")
        return jsonify({"error": str(e)}), 500
